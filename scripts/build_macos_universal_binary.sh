...
BIN_NAME="${1:-}"
if [[ -z "$BIN_NAME" ]]; then
  echo "usage: bash scripts/build_macos_universal_binary.sh <bin-name>" >&2
  exit 2
fi
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "error: macos_universal build requires macOS" >&2
  exit 1
fi
...
if [[ ! -f "$ARM_BIN" ]]; then
  echo "error: missing $ARM_BIN" >&2
  exit 2
fi
if [[ ! -f "$X86_BIN" ]]; then
  echo "error: missing $X86_BIN" >&2
  exit 2
fi
